...
export default class extends Component {
    ...
    formattedStartingNumber = () => {
        ...
        let {value, totalDuration, s, live} = this.props;
        ...
        if (s !== undefined) {
            ...
            value_ = moment.unix(value_).format(GLOBAL.Clock_Setting);
            var startingNumberValue_ = Math.round(startingNumberValue + s);
            startingNumberValue_ = moment
                .unix(startingNumberValue_)
                .format(GLOBAL.Clock_Setting);
            return scrubbing ? startingNumberValue_ : value_;
        } else {
            return scrubbing
    ...
    initiateAnimator = () => {
        this._translateX.addListener(({value}) => {
            ...
            this.setState({
                startingNumberValue:
                    (boundedValue / this.state.dimensionWidth) *
    ...
    onTap = ({nativeEvent}) => {
        if (nativeEvent.state === State.END && this.props.tapNavigation) {
            ...
            this.onSlidingComplete(
                (nativeEvent.x / dimensionWidth) * totalDuration,
            );
        }
    };
                        ...
                        >
                            ...
                            <Text
                                style={{
                                    ...
                                    position: 'absolute',
                                    bottom: 15,
                                    fontSize: GLOBAL.Device_IsAppleTV ? 22 : 14,
                                    width: 200,
                                }}
                            >
                        ...
                        >
                            {
                                this.props.s != undefined
                                    ? moment
                                          .unix(this.props.s)
                                          .format(GLOBAL.Clock_Setting)
                                    : '0:00' //this.formattedStartingNumber()
                            }
                        </Text>
                        <Text
                            style={[
