...
export default ({ navigation }): React.ReactElement => {
    ...
    const getUserIpFailBack = async () => {
        try {
            ...
            let data = await response.json();
            console.log('get user ip failback response: ', data);
            if (data != undefined) {
                GLOBAL.Device_IpAddress = data.ip;
                return getAccessToken();
            } else {
    ...
    const getAccessToken = async () => {
        try {
            var path =
                ...
                '.json';
            console.log('get access token: ', GLOBAL.HTTPvsHTTPS +
            ...
            try {
                console.log('authorize token: ', GLOBAL.HTTPvsHTTPS +
                ...
                console.log('authorize token response: ', data);
                if (!data_.CID) {
                    return {
                        success: false,
    ...
    const getUser = async token => {
        ...
        try {
            console.log('get user: ', GLOBAL.HTTPvsHTTPS +
                'authorize.akamaized.net/encrypt.php?CID=' +
                url +
                '&time=' +
                moment().unix());
    ...
    const getSettingsGui = async () => {
        ...
        try {
            console.log('get settings gui: ', path);
            let response = await fetch(path);
            let settings = await response.json();
            settings = JSON.parse(settings);
            console.log('get settings gui response: ', settings);
