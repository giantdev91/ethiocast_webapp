...
export default ({ navigation }): React.ReactElement => {
    ...
    const onSignIn = (): void => {
        ...
        (async () => {
            ...
            GLOBAL.show_log && console.log('authentication result ====> ', res);
            if (res.success) {
                var test = '';
    ...
    const getAccessToken = async () => {
        try {
            var path =
                ...
                '.json';
            GLOBAL.show_log && console.log('get access token: ', GLOBAL.HTTPvsHTTPS +
            ...
            try {
                GLOBAL.show_log && console.log('authorize token: ', GLOBAL.HTTPvsHTTPS +
                ...
                GLOBAL.show_log && console.log('authorize token response: ', data);
                if (!data_.CID) {
                    return {
                        success: false,
    ...
    const getUser = async token => {
        ...
        try {
            GLOBAL.show_log && console.log('get user: ', GLOBAL.HTTPvsHTTPS +
                'authorize.akamaized.net/encrypt.php?CID=' +
                url +
                '&time=' +
                moment().unix());
    ...
    const getSettingsGui = async () => {
        ...
        try {
            GLOBAL.show_log && console.log('get settings gui: ', path);
            let response = await fetch(path);
            let settings = await response.json();
            settings = JSON.parse(settings);
            GLOBAL.show_log && console.log('get settings gui response: ', settings);
        ...
        <View style={{ flex: 1 }}>
            ...
            <ScrollView
                ...
                <View
                    ...
                    <Text h4 center style={styles.signInLabel} status="control">
                        {LANG.getTranslation('signin')}
                    </Text>
                    <Text center style={styles.signInLabel_} status="basic">
                    ...
                    {GLOBAL.Support != '' ? (
                        <View
                            ...
                            <ScrollView
                                ...
                                <HTML
                                    source={{ html: Decode(GLOBAL.Support) }}
                                    baseFontStyle={{
                                        color: '#fff',
                                        fontSize: 16,
                                    }}
                ...
                <View
                    ...
                    <Button
                        ...
                        size="giant"
                        onPress={onSignIn}>
                        {authenticating ? '' : LANG.getTranslation('sign_in')}
                    </Button>
                </View>
