...
export default class Movies_Details extends Component {
    constructor(props) {
        ...
        GLOBAL.Selected_TagIndex = this.props.TagIndex;
    ...
    _onBack() {
        if (this.props.selected_tag != undefined) {
            Actions.Movies_Tags({
            ...
            // if (this.props.fromPage == 'Movies_Tags_Details') {
            //     Actions.Home();
            // } else {
            Actions.Movies_Details({
                MovieIndex: this.props.MovieIndex,
        ...
        DAL.getJson(path)
            .then(data => {
                if (data != undefined) {
                    ...
                    var desc = '';
                    if (moviedescriptions[0] != undefined) {
                        desc = moviedescriptions[0].description;
                    }
                    this.setState({
                        movie: movie,
                        title: name,
                    ...
                    DAL.validatePayPerView(
                        ...
                        if (data == true) {
                            rented = {
                                ...
                                end: moment().format('YYYY-MM-DDTHH:MM:00'),
    ...
    _onPressNext() {
        if (this.state.fromPage == 'Home') {
            if (
                ...
                GLOBAL.Selected_TagIndex
            ) {
                GLOBAL.Selected_TagIndex = GLOBAL.Selected_TagIndex + 1;
                this.updateStateByMovie();
            }
        } else if (this.state.fromPage == 'search') {
            if (GLOBAL.SearchMovies_.length - 1 > GLOBAL.Selected_TagIndex) {
                GLOBAL.Selected_TagIndex = GLOBAL.Selected_TagIndex + 1;
                this.updateStateByMovie();
            }
            ...
            if (GLOBAL.Movies.length - 1 > GLOBAL.Selected_TagIndex) {
                GLOBAL.Selected_TagIndex = GLOBAL.Selected_TagIndex + 1;
                this.updateStateByMovie();
            }
        }
        ...
        return (
            ...
            >
                ...
                >
                    <MovieList
                        FromPage={'Details'}
                        Movies={GLOBAL.Tags.find(t => t.tag == tag.item).movies}
                        Type={'Tags'}
                        ...
                        onPress={movie => this.openMovie(movie)}
                    />
                </View>
            </View>
        );
        ...
        if (getWatched == false) {
            ...
            return (
                <View>
                    ...
                    <View ref="header" style={styles.bt_view}>
                        <Image
                            style={{
                                width: this.state.movieimagewidth,
                                height: this.state.movieimagewidth * 1.5,
